...
export async function POST(req: NextRequest) {
  try {
    ...
    // Once the waiter accepts (status → pending) a new round order is created.
    if (body.tabId && body.type === 'dine-in') {
      ...
      if (existingRow) {
        ...
        // Insert new items under the existing order
        ...
        // Re-sync tab total across all live orders
        const { data: tabOrdsForSync } = await sb
          .from('orders')
          .select('total')
          .eq('tab_id', String(body.tabId))
          .not('status', 'in', '("cancelled","void")');
        const liveTotal = (tabOrdsForSync ?? []).reduce(
          (s: number, o: Record<string, unknown>) => s + Number(o.total), 0,
        );
        await sb.from('customer_tabs').update({ total: liveTotal }).eq('id', String(body.tabId));

        // Return updated order so the client gets fresh item list
        ...
        if (full) {
          ...
          await broadcast(rid, 'order_created', order); // triggers refresh in all portals (waiter/kitchen/customer)
