...
export default function KitchenPage() {
  ...
  // ── Supabase Realtime ────────────────────────────────────────────────────────
  // order_confirmed fires when a waiter taps "Confirm & Print" — this is the
  // primary event that brings a new order onto the kitchen display in the new
  // waiter-confirmation workflow. order_status_changed covers all other
  // transitions (preparing → prepared, etc.). Both trigger a full refresh so
  // the kitchen always sees the latest state from Supabase.
  useRealtime(
    process.env.NEXT_PUBLIC_RESTAURANT_ID ?? 'rest_default',
    {
      order_created:       () => { refresh(); },
      order_confirmed:     () => { refresh(); }, // waiter confirmed → enters kitchen as 'preparing'
